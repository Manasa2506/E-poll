...
def Home(root, frame1, frame2):
    ...
    Button(frame2, text="Home", command=lambda: Home(
        root, frame1, frame2)).grid(row=0, column=0)
    Label(frame2, text="                                                                         ").grid(
        row=0, column=1)
    Label(frame2, text="                                                                         ").grid(
        row=0, column=2)
    ...
    Label(frame1, text="Home", font=('Helvetica', 25, 'bold')).grid(
    ...
    # Voter Login
    voter = Button(frame1, text="Student Login", width=15,
                   command=lambda: voterLogin(root, frame1))

    # New Tab
    newTab = Button(frame1, text="New Window", width=15, command=lambda: sb_p.call(
        'start python homePage.py', shell=True))
