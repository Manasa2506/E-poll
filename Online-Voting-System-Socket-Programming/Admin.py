...
def AdminHome(root, frame1, frame3):
    ...
    Button(frame3, text="Admin", command=lambda: AdminHome(
        root, frame1, frame3)).grid(row=1, column=0)
    frame3.pack(side=TOP)

    Label(frame1, text="Admin", font=(
        'Helvetica', 25, 'bold')).grid(row=0, column=1)
    Label(frame1, text="").grid(row=1, column=0)

    # Admin Login
    runServer = Button(frame1, text="Run Server", width=15, command=lambda: sb_p.call(
        'start python Server.py', shell=True))

    # Voter Login
    registerVoter = Button(frame1, text="Register Student",
                           width=15, command=lambda: regV.Register(root, frame1))

    # Show Votes
    showVotes = Button(frame1, text="Show Votes", width=15,
...
def log_admin(root, frame1, admin_ID, password):
    ...
    if(admin_ID == "Admin" and password == "admin"):
        ...
        msg = Message(
            frame1, text="Either ID or Password is Incorrect", width=500)
...
def AdmLogin(root, frame1):
    ...
    Label(frame1, text="Admin Login", font=('Helvetica', 18, 'bold')).grid(
    ...
    sub = Button(frame1, text="Login", width=10, command=lambda: log_admin(
        root, frame1, admin_ID.get(), password.get()))
    Label(frame1, text="").grid(row=4, column=0)
