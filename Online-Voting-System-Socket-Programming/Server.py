...
def client_thread(connection):
    ...
    if(df.verify(log[0], log[1])):  # 3 Authenticate
        if(df.isEligible(log[0])):
            ...
            print('Vote Already Casted by:'+str(log[0]))
    ...
    print("Vote Received by: "+str(log[0])+"  Processing...")
    lock.acquire()
    # update Database
    if(df.vote_update(data.decode(), log[0])):
        print("Vote Casted Sucessfully by  = "+str(log[0]))
        connection.send("Successful".encode())
    else:
        print("Vote Update Failed by  = "+str(log[0]))
